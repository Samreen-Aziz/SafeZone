      ...
      <div class="wrap-login100">
				...
				<form class="login100-form validate-form" action = '' method = 'POST'>
					...
					<div class="wrap-input100 validate-input">
                        <h6>Driver Name</h6>
						<input class="input100" type="text" name="driver_name" id="driver_name" placeholder="Driver Name">
						<span class="focus-input100"></span>
					</div>
                        <button type = 'submit' name = 'submit' style = "margin-top: 25px;" class="login100-form-btn">
							Delete
						</button>
					</div>
				</form>
...
if (isset($_POST['submit'])) {
    ...
    // Update the student records with NULL values for the foreign key
    $updateQuery = "UPDATE student SET d_id = 0, d_prefix = 0 WHERE d_id = ? AND d_prefix = ?";
    $stmt = $mysqli->prepare($updateQuery);
    $stmt->bind_param('ss', $driverId, $driverPrefix);
    $stmt->execute();

    // Delete the driver record
    $deleteQuery = "DELETE FROM driver WHERE d_id = ? AND d_prefix = ?";
